    ...
    class SimplexService
    {
        ...
        public void PerformSimplex()
        {
            ...
            foreach (Event e in events)
            {
                foreach (Post p in e.Posts)
                {
                    if (p.MediaItem != null)
                    {
                        ...
                        attendees.Add(100); // eventually change so it gets the actual number of attendees for the event
            ...
            for (int i = 0; i < ids.Count; i++)
            {
                ...
                for (int j = 0; j < 3; j++)
                {
                    decs[i][j] = new Decision(Domain.IntegerNonnegative, String.Format("dec_{0}_{1}", j, ids[i]));
                }
                model.AddDecisions(decs[i][0], decs[i][1], decs[i][2]);
                model.AddConstraint(String.Format("con_{0}", ids[i]), decs[i][0] + decs[i][1] + decs[i][2] == 1);
            }
        ...
        private int GetScores(int attendees, DateTime created, DateTime eventDate)
        {
            ...
            if (eventDate > now)
            {
                int sinceCreated = (int)(now - created).TotalDays;
                int totalSpan = (int)(eventDate - created).TotalDays;
